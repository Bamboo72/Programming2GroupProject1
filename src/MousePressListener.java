...
class MousePressListener implements MouseListener {
    ...
    public void mouseClicked(MouseEvent event) {
        ...
        // System.out.println("Player at x: " + currentPlayerX + ", y: " +
        // currentPlayerY);
        ...
        if (TheIsleOfLaeso.g.diceRolled) { // Row = Y, Col = X, Note that I'm calling the top of the screen top/up
            ...
            if (TheIsleOfLaeso.collectsLeft > 0) { // If there are still collects left
                ...
                System.out.println("theResource: " + theResource + " so !theResource.equals(\"r \") is "
                        + !theResource.equals("r "));
                ...
                if (rowNum == currentPlayerY) { // Check the column
                    if (colNum == currentPlayerX + 1) {
                        ...
                        if (!theResource.equals("r ")) {
                            ...
                            System.out.println("On the board this is: "
                                    + TheIsleOfLaeso.i.getBoard()[rowNum][colNum].substring(2));
                            TheIsleOfLaeso.i.remove(TheIsleOfLaeso.i.getBoard()[rowNum][colNum].substring(2), colNum,
                                    rowNum);
            ...
            if (TheIsleOfLaeso.moveLeft > 0) { // If there are still moves left
                if (!TheIsleOfLaeso.i.getBoard()[rowNum][colNum].equals("o ")
                        && !TheIsleOfLaeso.i.getBoard()[rowNum][colNum].equals("n ")
                        && !Player.checkForPlayer(colNum, rowNum) && (Player.checkForStructure(colNum, rowNum) == null
                                || Player.checkForStructure(colNum, rowNum).owner == TheIsleOfLaeso.playerTurn)) {
                    if (colNum == currentPlayerX + 1) {
                        if (rowNum == currentPlayerY + 1) {
